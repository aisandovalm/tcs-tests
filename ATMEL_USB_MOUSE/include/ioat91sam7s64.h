...
// - *****************************************************************************
// - -------- VREG : (SYSC Offset: 0xd60) Voltage Regulator Mode Register -------- 
AT91C_SYSC_PSTDBY         EQU (0x1 <<  0) ;- (SYSC) Voltage Regulator Power Mode
...
// - *****************************************************************************
// - -------- SYSC_RCR : (RSTC Offset: 0x0) Reset Control Register -------- 
AT91C_SYSC_PROCRST        EQU (0x1 <<  0) ;- (RSTC) Processor Reset
AT91C_SYSC_ICERST         EQU (0x1 <<  1) ;- (RSTC) ICE Interface Reset
AT91C_SYSC_PERRST         EQU (0x1 <<  2) ;- (RSTC) Peripheral Reset
AT91C_SYSC_EXTRST         EQU (0x1 <<  3) ;- (RSTC) External Reset
AT91C_SYSC_KEY            EQU (0xFF << 24) ;- (RSTC) Password
// - -------- SYSC_RSR : (RSTC Offset: 0x4) Reset Status Register -------- 
AT91C_SYSC_URSTS          EQU (0x1 <<  0) ;- (RSTC) User Reset Status
AT91C_SYSC_BODSTS         EQU (0x1 <<  1) ;- (RSTC) Brown-out Detection Status
AT91C_SYSC_RSTTYP         EQU (0x7 <<  8) ;- (RSTC) Reset Type
AT91C_SYSC_RSTTYP_POWERUP EQU (0x0 <<  8) ;- (RSTC) Power-up Reset. VDDCORE rising.
AT91C_SYSC_RSTTYP_WATCHDOG EQU (0x2 <<  8) ;- (RSTC) Watchdog Reset. Watchdog overflow occured.
AT91C_SYSC_RSTTYP_SOFTWARE EQU (0x3 <<  8) ;- (RSTC) Software Reset. Processor reset required by the software.
AT91C_SYSC_RSTTYP_USER    EQU (0x4 <<  8) ;- (RSTC) User Reset. NRST pin detected low.
AT91C_SYSC_RSTTYP_BROWNOUT EQU (0x5 <<  8) ;- (RSTC) Brown-out Reset.
AT91C_SYSC_NRSTL          EQU (0x1 << 16) ;- (RSTC) NRST pin level
AT91C_SYSC_SRCMP          EQU (0x1 << 17) ;- (RSTC) Software Reset Command in Progress.
// - -------- SYSC_RMR : (RSTC Offset: 0x8) Reset Mode Register -------- 
AT91C_SYSC_URSTEN         EQU (0x1 <<  0) ;- (RSTC) User Reset Enable
AT91C_SYSC_URSTIEN        EQU (0x1 <<  4) ;- (RSTC) User Reset Interrupt Enable
AT91C_SYSC_ERSTL          EQU (0xF <<  8) ;- (RSTC) User Reset Enable
AT91C_SYSC_BODIEN         EQU (0x1 << 16) ;- (RSTC) Brown-out Detection Interrupt Enable

// - *****************************************************************************
// -              SOFTWARE API DEFINITION  FOR Real Time Timer Controller Interface
// - *****************************************************************************
// - -------- SYSC_RTMR : (RTTC Offset: 0x0) Real-time Mode Register -------- 
AT91C_SYSC_RTPRES         EQU (0xFFFF <<  0) ;- (RTTC) Real-time Timer Prescaler Value
AT91C_SYSC_ALMIEN         EQU (0x1 << 16) ;- (RTTC) Alarm Interrupt Enable
AT91C_SYSC_RTTINCIEN      EQU (0x1 << 17) ;- (RTTC) Real Time Timer Increment Interrupt Enable
AT91C_SYSC_RTTRST         EQU (0x1 << 18) ;- (RTTC) Real Time Timer Restart
// - -------- SYSC_RTAR : (RTTC Offset: 0x4) Real-time Alarm Register -------- 
AT91C_SYSC_ALMV           EQU (0x0 <<  0) ;- (RTTC) Alarm Value
// - -------- SYSC_RTVR : (RTTC Offset: 0x8) Current Real-time Value Register -------- 
AT91C_SYSC_CRTV           EQU (0x0 <<  0) ;- (RTTC) Current Real-time Value
// - -------- SYSC_RTSR : (RTTC Offset: 0xc) Real-time Status Register -------- 
AT91C_SYSC_ALMS           EQU (0x1 <<  0) ;- (RTTC) Real-time Alarm Status
AT91C_SYSC_RTTINC         EQU (0x1 <<  1) ;- (RTTC) Real-time Timer Increment

// - *****************************************************************************
// -              SOFTWARE API DEFINITION  FOR Periodic Interval Timer Controller Interface
// - *****************************************************************************
// - -------- SYSC_PIMR : (PITC Offset: 0x0) Periodic Interval Mode Register -------- 
AT91C_SYSC_PIV            EQU (0xFFFFF <<  0) ;- (PITC) Periodic Interval Value
AT91C_SYSC_PITEN          EQU (0x1 << 24) ;- (PITC) Periodic Interval Timer Enabled
AT91C_SYSC_PITIEN         EQU (0x1 << 25) ;- (PITC) Periodic Interval Timer Interrupt Enable
// - -------- SYSC_PISR : (PITC Offset: 0x4) Periodic Interval Status Register -------- 
AT91C_SYSC_PITS           EQU (0x1 <<  0) ;- (PITC) Periodic Interval Timer Status
// - -------- SYSC_PIVR : (PITC Offset: 0x8) Periodic Interval Value Register -------- 
AT91C_SYSC_CPIV           EQU (0xFFFFF <<  0) ;- (PITC) Current Periodic Interval Value
AT91C_SYSC_PICNT          EQU (0xFFF << 20) ;- (PITC) Periodic Interval Counter
// - -------- SYSC_PIIR : (PITC Offset: 0xc) Periodic Interval Image Register -------- 
...
// - *****************************************************************************
// - -------- SYSC_WDCR : (WDTC Offset: 0x0) Periodic Interval Image Register -------- 
AT91C_SYSC_WDRSTT         EQU (0x1 <<  0) ;- (WDTC) Watchdog Restart
// - -------- SYSC_WDMR : (WDTC Offset: 0x4) Watchdog Mode Register -------- 
AT91C_SYSC_WDV            EQU (0xFFF <<  0) ;- (WDTC) Watchdog Timer Restart
AT91C_SYSC_WDFIEN         EQU (0x1 << 12) ;- (WDTC) Watchdog Fault Interrupt Enable
AT91C_SYSC_WDRSTEN        EQU (0x1 << 13) ;- (WDTC) Watchdog Reset Enable
AT91C_SYSC_WDRPROC        EQU (0x1 << 14) ;- (WDTC) Watchdog Timer Restart
AT91C_SYSC_WDDIS          EQU (0x1 << 15) ;- (WDTC) Watchdog Disable
AT91C_SYSC_WDD            EQU (0xFFF << 16) ;- (WDTC) Watchdog Delta Value
AT91C_SYSC_WDDBGHLT       EQU (0x1 << 28) ;- (WDTC) Watchdog Debug Halt
AT91C_SYSC_WDIDLEHLT      EQU (0x1 << 29) ;- (WDTC) Watchdog Idle Halt
// - -------- SYSC_WDSR : (WDTC Offset: 0x8) Watchdog Status Register -------- 
AT91C_SYSC_WDUNF          EQU (0x1 <<  0) ;- (WDTC) Watchdog Underflow
AT91C_SYSC_WDERR          EQU (0x1 <<  1) ;- (WDTC) Watchdog Error
...
AT91C_SPI_CSAAT           EQU (0x1 <<  2) ;- (SPI) Chip Select Active After Transfer
...
AT91C_TC_EEVT_NONE        EQU (0x0 << 10) ;- (TC) Signal selected as external event: TIOB TIOB direction: input
AT91C_TC_EEVT_RISING      EQU (0x1 << 10) ;- (TC) Signal selected as external event: XC0 TIOB direction: output
AT91C_TC_EEVT_FALLING     EQU (0x2 << 10) ;- (TC) Signal selected as external event: XC1 TIOB direction: output
AT91C_TC_EEVT_BOTH        EQU (0x3 << 10) ;- (TC) Signal selected as external event: XC2 TIOB direction: output
...
AT91C_TC_ETRCS            EQU (0x1 <<  7) ;- (TC) External Trigger
AT91C_TC_ETRGS            EQU (0x1 << 16) ;- (TC) Clock Enabling
...
// - -------- TCB_BMR : (TCB Offset: 0xc4) TC Block Mode Register -------- 
AT91C_TCB_TC0XC0S         EQU (0x1 <<  0) ;- (TCB) External Clock Signal 0 Selection
...
AT91C_TCB_TC1XC1S         EQU (0x1 <<  2) ;- (TCB) External Clock Signal 1 Selection
...
AT91C_TCB_TC2XC2S         EQU (0x1 <<  4) ;- (TCB) External Clock Signal 2 Selection
...
AT91C_TCB_TC2XC2S_TIOA2   EQU (0x3 <<  4) ;- (TCB) TIOA2 connected to XC2
...
// - *****************************************************************************
// - ========== Register definition for SYSC peripheral ========== 
AT91C_SYSC_SYSC_VREG      EQU (0xFFFFFD60) ;- (SYSC) Voltage Regulator Mode Register
// - ========== Register definition for AIC peripheral ========== 
...
AT91C_SSC_RC0R            EQU (0xFFFD4038) ;- (SSC) Receive Compare 0 Register
...
AT91C_SSC_RC1R            EQU (0xFFFD403C) ;- (SSC) Receive Compare 1 Register
...
// -               BASE ADDRESS DEFINITIONS FOR AT91SAM7S64
// - *****************************************************************************
AT91C_BASE_SYSC           EQU (0xFFFFF000) ;- (SYSC) Base Address
